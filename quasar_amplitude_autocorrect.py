...
def main():
    ...
    while indata.table_highver('CL')>6:
        tab = indata.table_highver('CL')
        indata.zap_table('CL',tab)
    get_plots(indata)
    indata.zap_table('PL',-1)
    if not 'indata' in globals():
        globals().update({'indata':indata})
    if npols==1:
        W, baselines_key = read_in_data_1pol(indata)
    elif npols==2:
        W, baselines_key = read_in_data_2pol(indata)
    else:
        sys.exit('Unknown polarization configuration')
    polz = {}
    ...
    for ant in range(len(ant_key)):
        for freq in range(ifs):
            for pol in range(npols):
                if not cla=='SPLIT':
                    clcor(indata,freq+1,int(ant_key[ant]+1),clcorprm[ant][freq+pol*ifs],polz[pol])
                else:
...
def read_in_data_2pol(indata):
    ...
    for k in range(len(averaged_data)):
        ...
        dataR   = refine_fits(baselinefits1,7,'R',3,None)+refine_fits(baselinefits2,7,'R',3,None)
        dataL   = refine_fits(baselinefits1,7,'L',3,None)+refine_fits(baselinefits2,7,'L',3,None)
...
def clcor(data,bif,ant,clcorparm,polar):
