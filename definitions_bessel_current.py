...
# 2018/02/27 Added --no-check-certificate to get_eop         LH              # 
# 2020/07/01 Added mafringe3 for RR and LL seperate fringe            LH     #                                                               #
##############################################################################
...
# Download EOP file
#
def get_eop_old_old(eop_path):
    if os.path.exists(eop_path+'usno_finals.erp'):
        age = (time.time() - os.stat(eop_path+'usno_finals.erp')[8])/3600
        if age<12: pass
        else:
            os.popen(r'wget --no-check-certificate http://gemini.gsfc.nasa.gov/solve_save/usno_finals.erp')
...
def get_eop_old(eop_path):
    if os.path.exists(eop_path+'usno_finals.erp'):
        age = (time.time() - os.stat(eop_path+'usno_finals.erp')[8])/3600
        if age<12: pass
        else:
            os.popen(r'wget -N ftp://ftp.lbo.us/pub/staff/wbrisken/EOP/usno_finals.erp')
            os.popen(r' rm -rf '+eop_path+'usno_finals.erp')
            os.popen(r'mv usno_finals.erp '+eop_path)
    else:
        os.popen(r'wget -N ftp://ftp.lbo.us/pub/staff/wbrisken/EOP/usno_finals.erp')
...
def fringegeo(indata, refant):
    ...
    fringe.solint      = 6
    fringe.weightit    = 3
    fringe.aparm[1:]   = [2, 0, d3, 0, d5, 0, 10]
...
def make_baseline_plot(num_baselines, data, n, m, start, end, str_baselines, str_baselines2, n_blocks, block_times, f1):
        ...
        ax.set_xlim(start-over,end+over)
        if doplot_flag==1:
            ax.set_ylim(low_rate*1.3,high_rate*1.3)
        else:
            ax.set_ylim(-4,4)
        if m==1:
            title('Rate [mHz]')
...
def make_baseline_plot2(num_baselines, data, n, m, start, end, str_baselines, str_baselines2, n_blocks, block_times, f1):
        ...
        high_delay=max(max(data[n-1][:,1]), max(data[n-1][:,2]), max(data[n-1][:,3]), 1)
        low_delay =min(min(data[n-1][:,1]), min(data[n-1][:,2]), min(data[n-1][:,3]),-1)
        #if max(data[n-1][:,3]) < 0.2:
        #    high_delay=max(max(data[n-1][:,3])*1.2,0.1)
        #    low_delay=min(min(data[n-1][:,3])*1.2,-0.1)
        ax.plot(data[n-1][:,0],data[n-1][:,1],'.b',ms=size) # Delay
...
def fringecal(indata, fr_image, nmaps, refant, calsource,solint,smodel, doband, bpver, dpfour):
    ...
    fringe.aparm[7]    = 3  #remove this if you want good solutions
    fringe.dparm[1:]   = [1, 20, 50, 0]
    fringe.dparm[4]    = dpfour
    fringe.dparm[8]    = 0 
    fringe.nmaps       = nmaps
...
def mafringe(indata, fr_image, calsource, channel, refant, outdisk, doband, bpver,dpfour):
    ...
    splitdata = AIPSUVData(calsource, split.outcl, outdisk, 1)
    ...
    multidata = AIPSUVData(calsource, multi.outcl, outdisk, 1)
    ...
    vbglu.outname = calsource
    vbglu.outdisk = outdisk
    vbglu.outcl   = '2IF'
   
    vbgludata = AIPSUVData(calsource, '2IF', outdisk, 1)
    ...
    vbgludata2 = AIPSUVData(calsource, '4IF', outdisk, 1)
    ...
    fringe.calsour[1]    = ''
    fringe.solint        = 6
    fringe.aparm[1:]     = [2, 0, 1, 0, 0, 0, 4]
    fringe.dparm[1:]     = [1, -1, 0, 0]
    fringe.dparm[4]      = dpfour
    fringe.snver         = 0
    fringe()

    fringe.indata        = vbgludata2
    fringe()

    return multidata, vbgludata2
...
##############################################################################
#
def mafringe3(indata, fr_image, calsource, channel, refant, outdisk, doband, bpver,dpfour):
...
def plot_spot_map(inimg,indata):
    ...
    draw()
    imname = srcname+'.'+linename+'.spotmap.pdf'
#    show()
    savefig(imname,bbox_inches='tight')
#    os.popen(r'ps2pdf '+imname)
...
if ma_fringe_flag!=0 and line != cont:
    if line_data2.exists():
        line_data2.clrstat()
    ...
    if ma_fringe_flag==2:
        (outdata1,outdata2)=mafringe3(line_used, fr_image, calsource, channel, refant, line_data2.disk, doband, bpver, dpfour) 
    else:
