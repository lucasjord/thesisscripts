        program fit_geoblocks_iono

c       V4: changed to handle manual-phase cal'ed data:
c           changed model to reflect zero values for man-phs-cal scan
c           (see memo of 11/20/2012 on "Ionospheric Geoblock Analysis")
c       v5: free format reading of delay-rate file, hopefully to
c           be more immune to print format changes in AIPS versions

c       Uses ionospheric mapping function from TMS, instead of 
c       neutral atmospheric mapping function.

c       NB: flipped sign of fringe rate model for dispersive delay 
c           IONOS.FITS output has zenith delay signs flipped
c               compared to least-square solution internal printout 

c       Started with fit_geoblocks_v2c.f on Dec 6, 2010:  MJR

c	Input files:
c		1) "control_file.inp"   provides parameter controls, 
c                                                date & freq
c                                       NB: should not need to solve for
c                                           clock terms
...
c       Output file:
c           IONOS.FITS: Similar to ATMOS.FITS file, containing zenith
c                       dispersive delays.  Note needed to reverse sign
c                       of zenith delays to be consistent with CLCOR/IONO

	implicit real*8 ( a-h, o-z )

	real*8      new_params(96),param_sigmas(96),
     +		    params(96), param_incr(96), param_limits(96)
        character*8 parnames(96)
	integer     paramids(96)

     	real*8	    data(20000), model(20000), resids(20000),
     +		    res_err(20000), wtres(20000), gst(20000)

	real*8	    partls(20000,96)

        real*8      ut_min(5), ut_max(5), ut_blk(5)

	integer	    n_stat_a(20000), n_stat_b(20000),
     +              n_source(20000), n_block(20000)

        integer     num_per_stat_geo(12), num_per_stat_blk(12,5), 
     +              list_stations(12)
	...
	real*8	x(12), y(12), z(12), cal_ra(100), cal_dec(100)
	common/geometry/ x,y,z,gast0,cal_ra,cal_dec,num_stations

	luprint = 6
	write (luprint,1100)
1100	format (/30x,'Program fit_geoblocks_iono_v5: Nov 11, 2013')

c       Dimension Limits:
	max_num_data	=20000		! max number data equations
	num_params	=   96		! max number of parameters
	num_stations    =   12          ! max number of antennas
        max_blocks      =    5          ! max number of blocks
...
c                This line contains source coordinates...
     ...
     +                                        ra_deg,dec_deg,xepoch
		 ...
		 if ( n_cal.ge.1 .and. n_cal.le.100) then
		       cal_ra (n_cal) = ra_deg  * deg_rad            ! radians
		       cal_dec(n_cal) = dec_deg * deg_rad            ! radians
		    else
		       print *,' INVALID SU-TABLE SOURCE NUMBER (1-100 allowed):', n_cal
		       stop
		 endif
...
c===========================================================================   
	...
	real*8		params(96), param_limits(96), param_incr(96)
	character*8     parnames(96)
	integer		paramids(96)

	real*8          ut_min(5), ut_max(5)
...
c       -------------------
c       Zenith delay errors for up to 5 blocks of data...
...
c	Read in geodetic-like data for many calibrators...
	...
	integer      n_stat_geo(1000), n_src_geo(1000), n_blk_geo(1000)
...
c       ----------------------------------------------------------------
c                  Find manual phase-cal scan (time and block #)
        gst_old = -9.9d0                     ! rads
        del_min = 0.0022d0                   ! 30 sec in radians 
        d_min   = 9.9d9                      ! sec
        d_sum   = 0.d0
        n_sum   = 0
        n_blk_mpc = -1

        do n_geo = 1, num_geo_pts

           gst_new = gst_geo(n_geo)          ! rads

           if ( abs( gst_new - gst_old ).gt.del_min .or.
     +          n_geo.eq.num_geo_pts ) then

c              have new scan...process last values
               if ( n_sum .ge. 4 ) then

                  d_avg = d_sum / float(n_sum)

                  if ( d_avg .lt. d_min ) then

c                    record current scan with smallest delays
                     gst_mpc = gst_old                 ! radians
                     n_src_mpc = n_src_geo(n_geo-1)
                     n_blk_mpc = n_blk_geo(n_geo-1)
                     d_min = d_avg
...
c          accumulate delay values for current scan
           d_sum = d_sum + abs( delay_mb(n_geo) )
           n_sum = n_sum + 1

        enddo

        ut_days_mpc = (gst_mpc - gast0)/sidereal_rate/twopi       ! days
        write (6,3000) ut_days_mpc,n_blk_mpc,n_src_mpc
 3000   format(/' Manual phase-cal scan at UT(days)=',f12.7,
     +          ', in geoblock #',i2,', used source #',i3) 
...
c             get delay/rate for this geosource...
...
c             Next, get delay/rate for the manual phase cal source...
c             Change parameters that were set in above call to 
c             set_geo_parameters to those of the manual phase cal "mpc" scan

c             Manual phase cal source coordinates:
              ra_mpc  = cal_ra(n_src_mpc)
              dec_mpc = cal_dec(n_src_mpc)
              
c             Get zenith delays for "mpc" scan:
              n3  = (2 + n_blk_mpc)*num_stations    ! start of mpc block z-delays
              i_stat_a = n_stat_a(n_d)
              i_stat_b = n_stat_b(n_d)
              del_tau_z_a_mpc = params(n3+i_stat_a) ! cm of delay
              del_tau_z_b_mpc = params(n3+i_stat_b)

	      call calc_geo_model ( debug, gst_mpc, stm_mid, 
     +                  ra_mpc, dec_mpc, 
     +			x_a,y_a,z_a, x_b,y_b,z_b, 
     +			del_tau_z_a_mpc, del_tau_z_b_mpc,
     +                  clock_a, clock_b,
     +                  clock_rate_a, clock_rate_b,
     +                  clock_accel_a, clock_accel_b,
     +			tau_tot_cm_mpc, rate_Hz_mpc )

c             Calculate both delay & rate model and residual.
c             including subtracting the "mpc" scan values
c             First, a multi-band delay datum...
	      model(n_d)  = tau_tot_cm - tau_tot_cm_mpc   ! cm
	      resids(n_d) = data(n_d) - model(n_d)

c             Next, a fringe rate datum...
	      n_d = n_d + 1
	      model(n_d)  = rate_Hz - rate_Hz_mpc         ! Hz
	      resids(n_d) = data(n_d) - model(n_d)
...
c       -----------------------------------------------------------
c	Calculate delay error for geodetic cal data:

c	For station A...	
        del_tau_dot_a = 0.d0
	call iono_shift (debug, stm,stm_mid, 
     +                  ra,dec, x_a,y_a,z_a,		        
...
c	For station A...	
	call iono_shift (debug, stm_wiggled, stm_mid, 
     +                  ra,dec, x_a,y_a,z_a,		        
     +		        del_tau_z_a,del_tau_dot_a,
     +			del_tau_a_wiggled)                                   
...
c       ***Flip fringe-rate sign for dispersive delay***
        atm_rate_Hz = -atm_rate_Hz                            ! Hz
...
c===========================================================================	
        subroutine iono_shift(debug,stm,stm_mid,ra,dec,	
     +			      xm,ym,zm,del_tau_z,del_tau_dot,
     +			      tau_iono)

c	Returns excess ionospheric delay in radians of phase shift
c       for given station owing to a vertical path error in cm. 
 
c       Note that this phase shift does NOT take into account that
c       the phase shift is negative for a dispersive delay.  Will
c       take this into account in data calibration (CLCOR/IONO).

c       See Thompson, Moran, Swenson Eqs 13.139 and .140 
...
c	Calculate source zenith angle...
	cos_za = sin(stat_lat)*sin(dec) + 	
     +			cos(stat_lat)*cos(dec)*cos(sha)
        zen_ang_rad  = acos( cos_za )                   ! rad

c       Use mapping function approximation given in TMS Eq. 13.140...
        ro = 6370.d0           ! radius of earth (km)
        hi =  400.d0           ! height of ionosphere (km)

        zi = asin( (ro/(ro+hi)) * sin(zen_ang_rad) )    ! rad
        sec_zi = 1.d0 / cos( zi )

	del_tau_z_total = del_tau_z + del_tau_dot*(stm-stm_mid)*rad_hr    ! cm

        tau_cm = del_tau_z_total * sec_zi               ! cm

	tau_waves = tau_cm / wavelength			! wavelengths 
	tau_iono  = tau_waves * twopi                   ! radians of phase shift
...
c       Debug printout...
	if ( debug .gt. 3.d0 ) then
		...
		i_ut_sec= ut + 0.5
                zenith_angle = zen_ang_rad / deg_rad ! deg
                sec_za = 1.d0 / cos_za
...
c===========================================================================	
	...
	real*8	params(96), params_wiggled(96)
	real*8	partls(20000,96), gst(20000)
...
c             Get zenith delays for "mpc" scan:
     ...
     +			tau_mpc, rate_mpc )
              ...
              tau  = tau_src  - tau_mpc              ! cm
              rate = rate_src - rate_mpc             ! Hz

c             Now "wiggle" parameters and calculate numerical partials
	      do i_p = 1, num_params
...
C			by less than a value of 10^-4 (arcsec or cm)...
     ...
     +                          clock_rate_a, clock_rate_b,
     +                          clock_accel_a, clock_accel_b,
     +				tau_wiggled_src, rate_wiggled_src )


                       del_tau_z_a_mpc = params_wiggled(n3+i_stat_a) ! cm of delay
                       del_tau_z_b_mpc = params_wiggled(n3+i_stat_b)
     ...
     +                          clock_rate_a, clock_rate_b,
     +                          clock_accel_a, clock_accel_b,
     +                          tau_wiggled_mpc, rate_wiggled_mpc )

                       tau_wiggled  = tau_wiggled_src  - tau_wiggled_mpc  ! cm
                       rate_wiggled = rate_wiggled_src - rate_wiggled_mpc ! Hz

c                      Calculate multi-band delay partial
                       partls(n_d,i_p) = (tau_wiggled-tau)/del_param
     +                                      

c                      Calculate rate partial...
                       partls(n_d+1,i_p)=(rate_wiggled-rate)/del_param
...
c===========================================================================	
	...
	real*8		params(96), new_params(96)
	real*8		param_incr(96), param_limits(96)
	real*8		param_old(96)
...
c       ----------------------------------------
c       Write out 8 groups of station parameters...
	n_p = 0
	do n_param_groups = 1, 8
	   do n=1, num_stations	
	      n_p = n_p + 1
...
c===========================================================================	
	...
	real*8		params(96), param_sigmas(96)
	...
	character*32    control_file
        ...
        integer         flag(5), sequence(5), use_block_num(12,5)
...
c                Flip sign on zenith delay to be compatible with CLCOR/IONO **************
                 zen_del = -zen_del
...
C        THE SOLUTION AND CORRELATIONS ARE AUTOMATICALLY PRINTED OUT
      ...
      REAL*8       B(96,96),BSTORE(96,96),XIDENT(96,96),
     +             COR(96,96),ERROR(96),STDEV(96),PR(96),
     +             XHAT(96),SNEW(20000),PRODCT(20000)

      DIMENSION LL(96),MM(96)
      DATA      MSIZE /96/
...
c             the baseline data are correlated...
 ...
 2041	      FORMAT (2X,A8,5X,3(F13.6,5X),4X,A8)
	   enddo
        ...
        write (6,2042)
 2042   format(/' Note: will reverse signs of zenith delays in output',
     +          ' IONOS.FITS file to be compatible with CLCOR/IONO')
